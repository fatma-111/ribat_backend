"""
Rafiq Bot API — PRODUCTION v4.1
================================
Changes in v4.1 vs v4:
- REMOVED all embeddings / pgvector logic completely
- REMOVED all vector similarity search
- REPLACED RAG with PostgreSQL full-text search (tsvector / tsquery)
  with ILIKE fallback for short / non-Arabic queries
- Gemini is used ONLY for response generation (not embeddings)
- Knowledge base table simplified (no vector column)
- New helper: fts_knowledge_base() — clean retrieval, no external deps
- SQL schema updated: faq_knowledge_base uses tsvector GIN index
- All other endpoints, auth, analytics, PDF, FCM unchanged
"""
...
_T: dict[str, dict[str, str]] = {
    ...
    "missing_slot":           {"ar": "ابعت رقم الموعد — مثال: احجز sl_001",
                               "en": "Please send the slot number — example: book sl_001"},
    "slot_unavailable":       {"ar": "الموعد مش متاح. اختر ميعاد تاني.",
                               "en": "This slot is no longer available. Please choose another."},
    "booking_success":        {"ar": "تم الحجز ✅ رقم الحجز: ",       "en": "Booking confirmed ✅ Booking ID: "},
    "booking_details":        {"ar": "تفاصيل الحجز",                   "en": "Booking details"},
    "available_slots":        {"ar": "مواعيد متاحة",                   "en": "Available Slots"},
    ...
    "card_specialist":        {"ar": "مختص موصى به",                   "en": "Recommended Specialist"},
    "card_specialist_body_en": "Price: {price} EGP | Rating: {rating}",
    "card_specialist_body_ar": "السعر: {price} جنيه | التقييم: {rating}",
    "card_missing_booking":   {"ar": "ناقص بيانات الحجز",             "en": "Missing booking data"},
    "card_slot_unavailable":  {"ar": "الموعد غير متاح",               "en": "Slot unavailable"},
...
# ── Gemini (generation only — NO embeddings) ───────────────────────────
try:
...
def _run_schema_migrations() -> None:
    ...
    NOTE: No pgvector extension, no vector columns, no embedding indexes.
    faq_knowledge_base uses tsvector for full-text search.
    """
    if not DATABASE_URL:
    ...
    try:
        ...
        # ── faq_knowledge_base — NO vector column ──────────────────────
        # Plain text table with a tsvector column for fast full-text search.
        # The search_vector is maintained automatically by a trigger.
        cur.execute(
            """
        ...
        # GIN index for fast tsvector lookups — no pgvector required
        cur.execute(
            """
        ...
        # Backfill search_vector for any pre-existing rows that have NULL
        cur.execute(
            """
...
# ──────────────────────────────────────────────
# FULL-TEXT SEARCH — replaces all RAG / embedding retrieval
# ──────────────────────────────────────────────
...
def fts_knowledge_base(
    ...
    """
    Retrieve relevant KB entries using PostgreSQL full-text search.

    Strategy (in order):
      1. tsvector / tsquery search using 'simple' dictionary
         (works for both Arabic and English without language-specific dictionaries)
      2. ILIKE fallback — used when the tsquery returns zero results
         (short queries, single words, punctuation-heavy input)

    Returns a list of dicts: [{topic, question, answer, tags, rank}]
    No embeddings, no pgvector, no external API calls.
    """
    ...
    try:
        ...
        # We convert the raw query into a tsquery by:
        #   a) splitting on whitespace
        #   b) stripping punctuation tokens
        #   c) joining with ' | ' (OR) so partial matches still hit
        # Using 'simple' dictionary avoids the need for Arabic/English
        # language configs on the Postgres server.
        raw_tokens = [
            re.sub(r"[^\w\u0600-\u06FF]", "", tok)
            for tok in query.strip().split()
            if len(tok) >= 2
        ]
        # Remove empty strings that result from punctuation-only tokens
        ...
        if tokens:
            ...
            fts_sql = f"""
                ...
                LIMIT %s;
            """
            fts_params = [tsquery_str, tsquery_str] + params_fts + [limit]
        ...
        # ── 2. ILIKE fallback ──────────────────────────────────────────
        # Triggered when FTS returns nothing (e.g. very short query,
        # single rare word, or query with no matching tsvector tokens).
        if not results:
            # Use the first meaningful token or the full query for ILIKE
            ...
            """
            ilike_params = [
                like_pattern, like_pattern, like_pattern,  # WHERE
                like_pattern,                               # ORDER BY question priority
            ] + params_ilike + [limit]

            cur.execute(ilike_sql, ilike_params)
            rows = cur.fetchall()
        ...
        # Return empty list — caller will fall back to in-memory KB
        results = []
...
def fts_or_kb_fallback(
    ...
    """
    Try FTS first; if DB returns nothing, fall back to in-memory KB.
    Returns (results, from_db: bool).
    """
    db_results = fts_knowledge_base(query=query, topic=topic, lang=lang, limit=limit)
    if db_results:
        return db_results, True

    # In-memory KB fallback (always available, no DB required)
...
# ──────────────────────────────────────────────
# KNOWLEDGE BASE (in-memory — unchanged, used as fallback)
# ──────────────────────────────────────────────
...
# New: allows adding entries directly to faq_knowledge_base table
...
# ──────────────────────────────────────────────
# IN-MEMORY KB SEARCH (fallback, unchanged)
# ──────────────────────────────────────────────
...
FOLLOW_UP_BANK: Dict[str, List[str]] = {
    "anger":                 ["When does the anger peak most? (before bed / after school / screen time)",
                              "What usually happens in the 60 seconds before the outburst?"],
    "screen_addiction":      ["How many hours per day approximately? What mostly (YouTube/games/TikTok)?",
                              "Is there a specific time cutting off causes the biggest reaction?"],
    "teen_communication":    ["When is your teen most calm and open to talking?",
                              "Is it that they don't respond at all, or respond with anger?"],
    "bullying":              ["Where does the bullying happen most? (classroom/bus/club)",
                              "Is there any adult at school your child already trusts?"],
    "study_focus":           ["How many minutes can they focus before getting distracted?",
                              "Which subject creates the most resistance?"],
    "kids_stories":          ["How old is the child so I can pick the right story?",
                              "What theme do you prefer — honesty, sharing, courage, or respect?"],
    "activities_games":      ["Do you prefer a calm activity or something active and physical?",
                              "Do you have simple supplies like paper, pencils, or building blocks?"],
    "book_recommendations":  ["How old is your child and what kind of stories do they enjoy?",
                              "Values-based books or adventure stories?"],
    "assessment_personality":["Would you like to start a quick personality assessment?",
                              "How old is your child so I can tailor the questions?"],
    "general_parenting":     ["How old is your child?",
                              "When does the situation occur most often and what usually triggers it?"],
}
...
# ──────────────────────────────────────────────
# GEMINI HELPERS — generation only, NO embeddings
# ──────────────────────────────────────────────
...
def gemini_compose_answer(
    user_text: str,
    topic: str,
    retrieved_context: List[Dict[str, Any]],   # FTS results injected as context
    ...
    Retrieved KB context from PostgreSQL FTS is injected directly into the prompt —
    no embeddings, no vector search, no external retrieval API.
    """
    _require_gemini()
    ...
    if retrieved_context:
        ...
        for i, item in enumerate(retrieved_context, 1):
            context_lines.append(
                f"[KB {i}] Q: {item.get('question', '')} | A: {item.get('answer', '')}"
            )
        context_block = "\n".join(context_lines)
    else:
        context_block = "No specific knowledge base entries found. Use your general parenting knowledge."
    ...
    system = (
        ...
        "Use the RETRIEVED KNOWLEDGE BASE CONTEXT as your primary source for tips.\n"
        "If confidence < 65 or no context found: give a short empathetic reply + ONE follow-up question.\n"
        "If confidence >= 65: give 2-3 practical bullet points from the context + ONE follow-up + "
    ...
    full_prompt = (
        f"{system}\n\n"
        f"USER QUESTION:\n{user_text}\n\n"
        f"RETRIEVED KNOWLEDGE BASE CONTEXT (use this as primary source):\n{context_block}\n\n"
...
@app.get("/", tags=["System"])
def home():
    return {"status": "Rafiq running 🚀", "version": "4.1.0",
...
def health():
    return {
        ...
        "retrieval":      "postgres_fts",   # was "pgvector_embeddings"
...
def kb_search_api(topic: str, q: str = "", age: Optional[int] = None, lang: Optional[str] = None):
    ...
    Tries PostgreSQL FTS first; falls back to in-memory KB.
    """
    ...
    return {"topic": topic, "age": age, "matched": res.matched, "source": "in_memory_kb",
            "match_count": res.match_count, "used_default": res.used_default,
            "tips": res.tips}


@app.post("/kb/add", tags=["KB"])
def kb_add(req: KbAddRequest):
    """Add to in-memory KB (legacy endpoint — use /kb/faq/add for DB persistence)."""
...
def faq_kb_add(req: FaqKbAddRequest):
    ...
    The tsvector trigger updates search_vector automatically.
    """
    if req.admin_key != ADMIN_KEY:
        raise HTTPException(status_code=401, detail="Invalid admin_key")
    conn = get_conn()
...
# ──────────────────────────────────────────────
# ROUTES — CHAT (main) — FTS-powered retrieval
# ──────────────────────────────────────────────
...
def chat(req: ChatRequest):
    ...
    try:
        ...
        # ── FTS retrieval — replaces all embedding/pgvector calls ──────
        ...
        # Build a KbSearchResult for confidence scoring (backward compat)
        kb_res = KbSearchResult(
            tips=[{"id": f"fts_{i}", "tip": r.get("answer", ""), "topic": r.get("topic", topic),
                   "tags": r.get("tags", []), "age_min": 4, "age_max": 18}
                  for i, r in enumerate(retrieved_context)],
            matched=bool(retrieved_context),
            match_count=len(retrieved_context),
            used_default=not bool(retrieved_context),
        )
        ...
        try:
            # Pass FTS-retrieved context directly to Gemini — no embeddings needed
            final_text = intro + gemini_compose_answer(
                user_text=user_text,
                topic=topic,
                retrieved_context=retrieved_context,   # ← FTS results as context
                specialists=spec_list,
                slots=slots_list,
                memory=mem,
                followups=followups,
                confidence=conf,
        ...
        if ENABLE_VERIFY:
            verdict = gemini_verify_answer(user_text, final_text,
                {"topic": topic, "context": retrieved_context,
                 "specialists": spec_list, "slots": slots_list,
        ...
        for item in kb_res.tips:
            ...
            cards.append({"type": ctype, "title": ctitle, "body": item.get("tip") or item.get("answer", ""),
                          "meta": {"topic": topic, "age_used": age, "matched": kb_res.matched}})
...
def generate_parenting_plan(user_id: str):
    """Generate a personalised 30-day parenting plan (always in English)."""
    if not GEMINI_ENABLED or client is None:
...
def export_plan_pdf(user_id: str):
    """Export the latest parenting plan as a PDF (always English)."""
    if not _REPORTLAB_AVAILABLE:
